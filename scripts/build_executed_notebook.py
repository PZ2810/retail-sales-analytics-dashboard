"""
Execute Notebook Cells and Populate Outputs (Base64 Charts + Text)
Ensures eda_analysis.ipynb has rich pre-rendered outputs for recruiter review.
"""
...
df[['Order ID', 'Order Date', 'Customer Name', 'Category', 'Sub-Category', 'Sales', 'Profit', 'Discount']].head(3)"""
    ...
    out_1_text = f"Successfully loaded {df.shape[0]:,} transactions across {df.shape[1]} features.\n"
    head_html = df[['Order ID', 'Order Date', 'Customer Name', 'Category', 'Sub-Category', 'Sales', 'Profit', 'Discount']].head(3).to_html(classes='dataframe')
    
    c1 = nbf.v4.new_code_cell(code_1)
    c1.execution_count = 1
    c1.outputs = [
        nbf.v4.new_output(output_type='stream', name='stdout', text=out_1_text),
        nbf.v4.new_output(output_type='execute_result', execution_count=1, data={'text/html': head_html, 'text/plain': df[['Order ID', 'Order Date', 'Customer Name', 'Category', 'Sub-Category', 'Sales', 'Profit', 'Discount']].head(3).to_string()})
    ]
    cells.append(c1)

    # Section 2: Audit
    cells.append(nbf.v4.new_markdown_cell("""## 2. Data Cleaning & Feature Engineering Audit"""))
    code_2 = """# Statistical overview of key metrics
print("Summary Statistics:")
display(df[['Sales', 'Quantity', 'Discount', 'Profit', 'Profit Margin %', 'Ship Duration Days']].describe().round(2))"""
    desc_html = df[['Sales', 'Quantity', 'Discount', 'Profit', 'Profit Margin %', 'Ship Duration Days']].describe().round(2).to_html(classes='dataframe')
    c2 = nbf.v4.new_code_cell(code_2)
    c2.execution_count = 2
    c2.outputs = [
        nbf.v4.new_output(output_type='stream', name='stdout', text="Summary Statistics:\n"),
        nbf.v4.new_output(output_type='execute_result', execution_count=2, data={'text/html': desc_html, 'text/plain': df[['Sales', 'Quantity', 'Discount', 'Profit', 'Profit Margin %', 'Ship Duration Days']].describe().round(2).to_string()})
    ]
    cells.append(c2)
...
*How do sales and profitability evolve over time, and what seasonal patterns exist?*"""))
    ...
    code_3 = """monthly_trend = df.groupby('Order YearMonth').agg(
...
plt.show()"""
    ...
    plt.title('Monthly Sales & Profit Trend (2014 - 2017): Strong Q4 Surges & Consistent Growth', 
              fontsize=14, fontweight='bold', pad=15)
    fig.tight_layout()
    chart1_path = os.path.join("notebook", "charts", "01_monthly_trend.png")
    fig.savefig(chart1_path, bbox_inches='tight', dpi=150)
    buf = io.BytesIO()
    ...
    c3.outputs = [
        nbf.v4.new_output(output_type='display_data', data={'image/png': img_b64_1, 'text/plain': '<Figure size 2100x825 with 2 Axes>'})
    ]
    cells.append(c3)
...
*Which sub-categories are the real profit drivers, and which are margin drains?*"""))
    ...
    code_4 = """subcat_perf = df.groupby(['Category', 'Sub-Category']).agg(
    Total_Sales=('Sales', 'sum'),
    Total_Profit=('Profit', 'sum')
...
plt.show()"""
    ...
    bars = ax.barh(subcat_perf['Sub-Category'], subcat_perf['Total_Profit'], color=colors, height=0.65)
    ...
    ax.set_title('Net Profit by Sub-Category: Severe Drain in Tables, Bookcases & Supplies', 
                 fontsize=14, fontweight='bold', pad=15)
    ax.set_xlabel('Cumulative Net Profit ($)', fontweight='bold', labelpad=10)
    ax.set_ylabel('Sub-Category', fontweight='bold', labelpad=10)
    ax.set_xlim(-25000, 65000)
    fig.tight_layout()
    chart2_path = os.path.join("notebook", "charts", "02_subcategory_profit.png")
    fig.savefig(chart2_path, bbox_inches='tight', dpi=150)
    buf = io.BytesIO()
    fig.savefig(buf, format='png', bbox_inches='tight', dpi=150)
    buf.seek(0)
    ...
    c4.outputs = [
        nbf.v4.new_output(output_type='display_data', data={'image/png': img_b64_2, 'text/plain': '<Figure size 1800x1050 with 1 Axes>'})
    ]
    cells.append(c4)
...
*At what point does discounting destroy net margin, and does higher volume compensate?*"""))
    ...
    code_5 = """discount_tiers = df.groupby('Discount Tier').agg(
...
plt.show()"""
    ...
    fig.tight_layout()
    chart3_path = os.path.join("notebook", "charts", "03_discount_cliff.png")
    fig.savefig(chart3_path, bbox_inches='tight', dpi=150)
    buf = io.BytesIO()
    fig.savefig(buf, format='png', bbox_inches='tight', dpi=150)
    buf.seek(0)
    ...
    c5.outputs = [
        nbf.v4.new_output(output_type='display_data', data={'image/png': img_b64_3, 'text/plain': '<Figure size 2250x825 with 2 Axes>'})
    ]
    cells.append(c5)
...
*Which regions excel, and why is the Central region lagging in profitability?*"""))
    ...
    code_6 = """reg_perf = df.groupby('Region').agg(
    Total_Sales=('Sales', 'sum'),
    Total_Profit=('Profit', 'sum')
).reset_index()
...
plt.show()"""
    ...
    fig.tight_layout()
    chart4_path = os.path.join("notebook", "charts", "04_regional_performance.png")
    fig.savefig(chart4_path, bbox_inches='tight', dpi=150)
    buf = io.BytesIO()
    fig.savefig(buf, format='png', bbox_inches='tight', dpi=150)
    buf.seek(0)
    ...
    c6.outputs = [
        nbf.v4.new_output(output_type='display_data', data={'image/png': img_b64_4, 'text/plain': '<Figure size 1800x825 with 2 Axes>'})
    ]
    cells.append(c6)
...
*Which customer segment represents the highest value and efficiency?*"""))
    ...
    code_7 = """segment_perf = df.groupby('Segment').agg(
    Total_Sales=('Sales', 'sum'),
    Total_Profit=('Profit', 'sum'),
...
plt.show()"""
    ...
    ax2.set_ylim(0, 560)
    fig.tight_layout()
    chart5_path = os.path.join("notebook", "charts", "05_segment_economics.png")
    fig.savefig(chart5_path, bbox_inches='tight', dpi=150)
    buf = io.BytesIO()
    fig.savefig(buf, format='png', bbox_inches='tight', dpi=150)
    buf.seek(0)
    img_b64_5 = base64.b64encode(buf.read()).decode('utf-8')
    plt.close(fig)

    c7 = nbf.v4.new_code_cell(code_7)
    c7.execution_count = 7
    c7.outputs = [
        nbf.v4.new_output(output_type='display_data', data={'image/png': img_b64_5, 'text/plain': '<Figure size 2100x780 with 2 Axes>'})
    ]
    cells.append(c7)
...
*Notebook generated and validated as part of the Retail Sales Analytics Portfolio Project.*"""))
    ...
    with open(notebook_path, 'w', encoding='utf-8') as f:
        nbf.write(nb, f)
    print(f"Successfully generated pre-rendered notebook at {notebook_path}.")
